...
{
    public static class DelaunayGenericAlgorithm
    {
        ...
        public static void create_constrained_mesh(int the_surface_index, List<int> inner_surface_indices, ref List<PSLGDataStructure.SurfaceStore> the_surface_data, float h, bool refine)
        {
            ...
            List<PSLGDataStructure.edge2d>[] inner_edges = new List<PSLGDataStructure.edge2d>[inner_surface_indices.Count]; // variable to store inner surfaces edges
            ...
            for (int i = 0; i < inner_surface_indices.Count; i++)
            {
                ...
                inner_edges[i] = new List<PSLGDataStructure.edge2d>();
            ...
            List<PSLGDataStructure.point2d> all_pts = new List<PSLGDataStructure.point2d>(); // variable to store all the node
            foreach (PSLGDataStructure.edge2d o_ed in outter_edges) // add the outter edges pts
            {
                all_pts.Add(new PSLGDataStructure.point2d(all_pts.Count, o_ed.end_pt.x, o_ed.end_pt.y)); // just add the end point of the edges (Since the edges are closed => adding end point alone covers all the surface nodes)
            }
            for (int i = 0; i < inner_surface_indices.Count; i++) // add the inner edges pts
            {
                foreach (PSLGDataStructure.edge2d i_ed in inner_edges[i]) // add the i_th inner edges pts
                {
                    all_pts.Add(new PSLGDataStructure.point2d(all_pts.Count, i_ed.end_pt.x, i_ed.end_pt.y)); // just add the end point of the edges (Since the edges are closed => adding end point alone covers all the surface nodes)
                }
            }
            ...
            do
            {
                ...
                List<PSLGDataStructure.point2d> amend_nodes = new List<PSLGDataStructure.point2d>();
            ...
            the_surface_data[the_surface_index].my_mesh = new PSLGDataStructure.mesh2d(main_mesh.local_input_points, main_mesh.local_output_edges, main_mesh.local_output_triangle);
            ...
            {
                the_surface_data[the_surface_index].encapsulating_seed_edges = new List<PSLGDataStructure.edge2d>();
            ...
            for (int i = 0; i < inner_surface_indices.Count; i++) // cycle through all the inner surfaces
            {
                ...
                {
                    the_surface_data[inner_surface_indices[i]].encapsulating_seed_edges = new List<PSLGDataStructure.edge2d>();
        ...
        {
            List<PSLGDataStructure.edge2d> the_edges = new List<PSLGDataStructure.edge2d>(); // the edges is the return variable
            if (the_surface.encapsulating_seed_edges.Count == 0)
            {
                ...
                foreach (PSLGDataStructure.edge2d ed in the_surface.surface_edges)
                {
            ...
            else
            {
                ...
                foreach (PSLGDataStructure.edge2d ed in the_surface.encapsulating_seed_edges)
                {
        ...
        public static void encroched_segment_recursive_divide(ref List<PSLGDataStructure.edge2d> surface_edges, int end_index, int start_index, ref List<PSLGDataStructure.point2d> all_nodes, ref List<PSLGDataStructure.point2d> amend_nodes, ref bool to_continue, ref bool seed_exists)
        {
            if (seed_exists == false)
            {
                ...
                for (int i = end_index; i >= start_index; i--) // go reverse to avoid overflow error
                {
                    ...
                    PSLGDataStructure.point2d cicle_center = new PSLGDataStructure.point2d(-1, surface_edges[i].mid_pt.x, surface_edges[i].mid_pt.y); // circle center
                    double circle_radius_squared = Math.Pow((surface_edges[i].edge_length * 0.5) - eps, 2);

                    foreach (PSLGDataStructure.point2d pt in all_nodes)
                    {
                        ...
                        if ((Math.Pow((pt.x - cicle_center.x), 2) + Math.Pow((pt.y - cicle_center.y), 2)) < circle_radius_squared)
                        {
                            ...
                            PSLGDataStructure.point2d split_pt = new PSLGDataStructure.point2d(all_nodes.Count + amend_nodes.Count, surface_edges[i].mid_pt.x, surface_edges[i].mid_pt.y);
                            ...
                            List<PSLGDataStructure.edge2d> two_segments = surface_edges.FindAll(obj => obj.start_pt.Equals(pt) || obj.end_pt.Equals(pt));
                            if (two_segments.Count != 0)
                            {
                                // if (two_segments.Count >2) // Some thing is terribly wrong
                                PSLGDataStructure.point2d other_pt1 = two_segments[0].start_pt.Equals(pt) == true ? two_segments[0].end_pt : two_segments[0].start_pt;
                                PSLGDataStructure.point2d other_pt2 = two_segments[two_segments.Count - 1].end_pt.Equals(pt) == true ? two_segments[two_segments.Count - 1].start_pt : two_segments[two_segments.Count - 1].end_pt;

                                if (surface_edges[i].vertex_exists(other_pt1) == true || surface_edges[i].vertex_exists(other_pt2) == true)
                                {
                                    PSLGDataStructure.point2d apex_pt, non_apex_vertex_seg1, non_apex_vertex_seg2;
                                    ...
                                    if (Display.StaticClass.GetAngle(non_apex_vertex_seg1.x, non_apex_vertex_seg1.y, apex_pt.x, apex_pt.y, non_apex_vertex_seg2.x, non_apex_vertex_seg2.y) < 0.5236)
                                    {
                                        ...
                                        if (t_param > 0.5f)
                                        {
                                            ...
                                            split_pt = new PSLGDataStructure.point2d(all_nodes.Count + amend_nodes.Count, split_x, split_y);
                                        }
                                    }
                                }
                            }
                            ...
                            PSLGDataStructure.edge2d seg_1 = new PSLGDataStructure.edge2d(surface_edges[i].edge_id, surface_edges[i].start_pt, split_pt); // create a segment 1 with start_pt to mid_pt
                            PSLGDataStructure.edge2d seg_2 = new PSLGDataStructure.edge2d(surface_edges.Count, split_pt, surface_edges[i].end_pt); // create a segment 2 with mid_pt to end_pt
                            amend_nodes.Add(new PSLGDataStructure.point2d(all_nodes.Count + amend_nodes.Count, split_pt.x, split_pt.y));
        ...
        public static void encroched_segment_single_divide(ref List<PSLGDataStructure.edge2d> surface_edges, ref PSLGDataStructure.point2d circumcenter_pt, double meansize, ref bool to_continue, ref bool seed_exists)
        {
            if (seed_exists == false)
            {
                ...
                for (int i = surface_edges.Count - 1; i >= 0; i--) // go reverse to avoid overflow error
                {
                    ...
                    if ((Math.Pow((circumcenter_pt.x - cicle_center.x), 2) + Math.Pow((circumcenter_pt.y - cicle_center.y), 2)) < circle_radius_squared)
                    {
                        ...
                        PSLGDataStructure.point2d c_pt = circumcenter_pt;
                        if (surface_edges.Exists(obj => obj.start_pt.Equals(c_pt) || obj.end_pt.Equals(c_pt)) == true)
                        {
                            ...
                            // Find the two segments connected to the point
                            List<PSLGDataStructure.edge2d> two_segments = surface_edges.FindAll(obj => obj.start_pt.Equals(c_pt) || obj.end_pt.Equals(c_pt));

                            // if (two_segments.Count >2) // Some thing is terribly wrong
                            PSLGDataStructure.point2d other_pt1 = two_segments[0].start_pt.Equals(c_pt) == true ? two_segments[0].end_pt : two_segments[0].start_pt;
                            PSLGDataStructure.point2d other_pt2 = two_segments[two_segments.Count - 1].end_pt.Equals(c_pt) == true ? two_segments[two_segments.Count - 1].start_pt : two_segments[two_segments.Count - 1].end_pt;
                            ...
                            if (surface_edges[i].vertex_exists(other_pt1) == true || surface_edges[i].vertex_exists(other_pt2) == true)
                            {
                                ...
                                if (Display.StaticClass.GetAngle(non_apex_vertex_seg1.x, non_apex_vertex_seg1.y, apex_pt.x, apex_pt.y, non_apex_vertex_seg2.x, non_apex_vertex_seg2.y) < 0.5236)
                                {
                                    ...
                                    if (t_param > 0.5f)
                                    {
                                        ...
                                        circumcenter_pt = new PSLGDataStructure.point2d(-1, split_x, split_y);

                                        PSLGDataStructure.edge2d tseg_1 = new PSLGDataStructure.edge2d(surface_edges[i].edge_id, surface_edges[i].start_pt, circumcenter_pt); // create a segment 1 with start_pt to mid_pt
                                        PSLGDataStructure.edge2d tseg_2 = new PSLGDataStructure.edge2d(surface_edges.Count, circumcenter_pt, surface_edges[i].end_pt); // create a segment 2 with mid_pt to end_pt
                    ...
                    half_split:;
                        ...
                        // create two segements from one segment by splitting at the mid point
                        PSLGDataStructure.edge2d seg_1 = new PSLGDataStructure.edge2d(surface_edges[i].edge_id, surface_edges[i].start_pt, surface_edges[i].mid_pt); // create a segment 1 with start_pt to mid_pt
                        PSLGDataStructure.edge2d seg_2 = new PSLGDataStructure.edge2d(surface_edges.Count, surface_edges[i].mid_pt, surface_edges[i].end_pt); // create a segment 2 with mid_pt to end_pt
                        circumcenter_pt = new PSLGDataStructure.point2d(-1, surface_edges[i].mid_pt.x, surface_edges[i].mid_pt.y);
        ...
        public static bool triangle_angle_size_constraint(PSLGDataStructure.SurfaceStore the_surface, mesh_store.triangle_store graded_triangle, double B, double h)
        {
                        ...
                        the_surface.pointinsurface(graded_triangle.shrunk_vertices[2].x, graded_triangle.shrunk_vertices[2].y) == true)
            {
                ...
                // more conditions if any before calling final else
            }
            else
            {
                return false;
        ...
        public static void delete_constrained_mesh(int the_surface_index, List<int> inner_surface_indices, ref List<PSLGDataStructure.SurfaceStore> the_surface_data)
        {
            // delete the mesh
        ...
        {
            //#### Output Variables #######
            public List<PSLGDataStructure.point2d> local_input_points = new List<PSLGDataStructure.point2d>();
            public List<PSLGDataStructure.edge2d> local_output_edges = new List<PSLGDataStructure.edge2d>();
            public List<PSLGDataStructure.triangle2d> local_output_triangle = new List<PSLGDataStructure.triangle2d>();
            //#####################################
            ...
            public void Add_multiple_points(List<PSLGDataStructure.point2d> parent_inpt_points)
            {
                // Call this first
                this._all_points = new List<point_store>();

                // Transfer the parent variable to local point_store list
                foreach (PSLGDataStructure.point2d pt in parent_inpt_points)
                {
            ...
            public void Add_single_point(PSLGDataStructure.point2d parent_onemore_point)
            {
                // don't call this before calling Add_multiple_points
            ...
            public void Finalize_mesh(PSLGDataStructure.SurfaceStore the_surface)
            {
                ...
                local_output_edges = new List<PSLGDataStructure.edge2d>();
                local_output_triangle = new List<PSLGDataStructure.triangle2d>();
                ...
                foreach (triangle_store the_tri in this.all_triangles)
                {
                    ...
                    PSLGDataStructure.triangle2d temp_tri = new PSLGDataStructure.triangle2d(i, the_tri.pt1.get_parent_data_type, the_tri.pt2.get_parent_data_type, the_tri.pt3.get_parent_data_type);
                ...
                foreach (edge_store the_edge in this.all_edges)
                {
                    ...
                    PSLGDataStructure.edge2d temp_edge = new PSLGDataStructure.edge2d(i, the_edge.start_pt.get_parent_data_type, the_edge.end_pt.get_parent_data_type);
            ...
            private void flip_bad_edges(int tri_index, point_store pt)
            {
                ...
                //       al/ || \bl            al/    \bl        /   \
                //        /  ||  \              /      \      al/     \bl
                //       /  a||b  \    flip    /___a____\      /___a___\  
                //     p0\   ||   /pt   =>   p0\---b----/pt  p0\---b---/pt  
                //        \  ||  /              \      /        \     /
                //       ar\ || /br            ar\    /br      ar\   /br
                //          \||/                  \  /            \ /
                //           p2                    p2              p2
            ...
            private void set_bounding_triangle(List<point_store> all_input_vertices)
            {
                ...
                // set the vertex
                this.s_p1 = new point_store(pt_count + 1, 0, Math.Round(k / 2.0f), new PSLGDataStructure.point2d(pt_count + 1, x_zero, -k));
                this.s_p2 = new point_store(pt_count + 2, Math.Round(k / 2.0f), 0.0, new PSLGDataStructure.point2d(pt_count + 2, k, y_zero));
                this.s_p3 = new point_store(pt_count + 3, -1 * Math.Round(k / 2.0f), -1 * Math.Round(k / 2.0f), new PSLGDataStructure.point2d(pt_count + 3, -k, k));
            ...
            public class point_store
            {
                ...
                double _x;
                double _y;
                PSLGDataStructure.point2d parent_pt;
                ...
                public PSLGDataStructure.point2d get_parent_data_type
                {
                    get { return parent_pt; }
                }

                public point_store(int i_pt_id, double i_x, double i_y, PSLGDataStructure.point2d pt)
                {
                    // Constructor
                    // set id
                    this._pt_id = i_pt_id;
            ...
            public class triangle_store
            {
                ...
                private void set_incircle()
                {
                    ...
                    this._circum_center = new point_store(-1, center_x, center_y, new PSLGDataStructure.point2d(-100, center_x, center_y));
